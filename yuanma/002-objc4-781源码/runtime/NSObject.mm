...
static StripedMap<SideTable>& SideTables() {
    return SideTablesMap.get();//这里得到的是所有的sideTable表，真机下有8张
}
...
#   define POOL_BOUNDARY nil
    ...
    {
        //获取当前页，如果当前页存在，就压栈对象，如果不存在，就创建页
        AutoreleasePoolPage *page = hotPage();
...
objc_object::clearDeallocating_slow()
{
    ...
    if (isa.has_sidetable_rc) {
        table.refcnts.erase(this);//删除sideTable表
    }
    table.unlock();
}
...
{
    if (!obj) return obj;
    if (obj->isTaggedPointer()) return obj;//此处可以看到如果是小对象类型是不会release的
