...
*   (e.g. method list fixup markers)
 这是一个可以存放通用数据的数组结构，什么是non-fragile？？？
 元素必须是结构体类型
 列表是entsize_list_tt结构的实例化后的结构名称，
 FlagMask用于在entsize字段中隐藏额外的位
**********************************************************************/
...
* count/begin/end iterate the underlying metadata elements
 ...
 元素是基础元数据类型
 List是元数据的列表类型
 这个结构体有三种值：1、空的；2、指向单个列表的指针；3、指向列表的指针数组
 ...
 public:
    ...
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        //如果存在，且只有第一个，
            ...
            array()->count = newCount;
            //将旧方法列表放到最后的一个位置，这里的addedCount自然就是总数-1了
...
/*
 成员包括ro_or_rw_ext，其他的数据没有
 说明rw本身不存储数据，而是存储ro和rw_ext
 可以通过方法获取ro或rw_ext，如果存在rwe就会先获取rwe，如果不不存在，才去获取ro
 */
struct class_rw_t {
...
private:
    //这里是一个PointerUnion结构体，存储有ro和rwe
...
struct objc_class : objc_object {//继承自objc_object,充分证明万物皆对象
    ...
    //是否存在alloc/allocWithZone方法，默认会有
    ...
    const char *mangledName() { 
        ...
        //如果已经实现，直接在ro中获取
        //如果还没有实现，则通过data()直接拿到类名（需要先强转成ro)
    ...
    // May be unaligned depending on class's ivars.
    //第三步：拿到类的信息的大小
    //创建空间，重要的是加载类的结构，类的结构中包含了方法列表、协议列表、属性列表等内容，这些东西就放在了ro中
    //所以加载数据段的时候(这个数据段就是类结构)，重要的就是ro
    //此处通过ro得到isa
    uint32_t unalignedInstanceSize() const {
        ASSERT(isRealized());
        //这里可以看到只拿到了ro的isa，也就是8个字节，
        return data()->ro()->instanceSize;
    }

    // Class's ivar size rounded up to a pointer-size boundary.
    //第二步：没有字节对齐的东西进行一下字节对齐
    ...
    size_t instanceSize(size_t extraBytes) const {
        ...
        //第一步拿到字节对齐实例size
        //alignedInstanceSize()得到的是：字节对齐实例size
...
struct category_t {
    ...
    // Fields below this point are not always present on disk.
    //这些字段不一定会存在于内存中，会根据MachO信息来判断
    struct property_list_t *_classProperties;//类属性列表，先不用关注

    //方法列表
    /*
     从中可以看到虽然命名为元类的方法列表，但是会自行判断，
     如果是元类则返回类方法，如果不是元类返回实例方法
     其实很多地方都是这样的。。。
     */
