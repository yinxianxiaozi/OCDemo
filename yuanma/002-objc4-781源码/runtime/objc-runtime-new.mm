...
static Class realizeClassWithoutSwift(Class cls, Class previously)
{
    ...
    if (!cls) return nil;
    //是否这个类是存在的，也就是已经实现了的，如果已经有了则直接返回
    ...
    if (ro->flags & RO_FUTURE) {
        ...
        rw = objc::zalloc<class_rw_t>();//创建rw
        rw->set_ro(ro);
...
* Locking: runtimeLock acquired by map_images or objc_readClassPair
 读取一个被编译器写入的类或元类
 返回新的类指针，结果可能是一个类，nil，获取其他
 请注意，此函数执行的所有工作都由mustReadClasses（）预引导。在不更新该函数的情况下，请勿更改该函数。
 
 做了两件事：
 1、Adds name => cls to the named non-meta class map
 2、Add a class and metaClass to the table of all classes
**********************************************************************/
...
# endif
    ...
    //1、修复selector引用，将不同库的selector重新设置地址
...
// cls must already be connected.递归调用cls和任何未加载的父类
/*
